...
def equal_opportunity_difference(dataset, true_labels, predictions, attribute, reference=None, target_value=None):
    ...
    Output: a negative EOD is a bias against an attribute class, while a positive SPD is a bias *for* that attribute
     class.
    """
    ...
    for comp in attribute_components:
        ...
        if comp == reference:
            ...
            _, _, fn1, tp1 = confusion_matrix(comp_dataset['true_labels'], comp_dataset['predictions']).ravel()
            true_pos_rate_comp = (tp1 / (tp1 + fn1))
            _, _, fn2, tp2 = confusion_matrix(ref_dataset['true_labels'], ref_dataset['predictions']).ravel()
            true_pos_rate_ref = (tp2 / (tp2 + fn2))
            eod = true_pos_rate_comp - true_pos_rate_ref
            print(f"The Equal Opportunity Difference for the protected attribute {comp} is {eod}.")
