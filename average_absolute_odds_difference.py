...
Average Absolute Odds Difference (AAOD) = 1/2 * [|FPR(minority) - FPR(reference/majority)| + |TPR(minority) - TPR(reference/majority)|]
...
Output: An AAOD value closer to zero is an indicator of the model being more fair, with an AAOD of zero is true fairness.
...
def average_absolute_odds_difference(dataset, true_labels, predictions, attribute, reference=None, target_value=None):
    ...
    true_neg_ref, false_pos_ref, false_neg_ref, true_pos_ref = confusion_matrix(ref_dataset['true_labels'], ref_dataset['predictions']).ravel()
    ...
    for comp in attribute_components:
        ...
        if comp == reference:
            ...
            print("The Equal Opportunity Difference for the protected attribute ", comp, " is ", avg_abs_odds_diff)
